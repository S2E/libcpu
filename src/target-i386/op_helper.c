...
void helper_emms(void) {
    /* set to empty state */
#if defined(CONFIG_SYMBEX) && !defined(SYMBEX_LLVM_LIB)
    WR_cpu(env, fptags, 0x0101010101010101ULL);
#else
...
uint8_t RR_cpu_float_status(void* p, unsigned size) {
    assert(size == 1 && "unexpected status field width");

    if ((void*)env < p && (p - (void*)env) < offsetof(CPUArchState, eip)) {
        return (uint8_t)__RR_env_raw(env, p - (void*)env, size);
    }
    assert(false && "unexpected calling context");
    return *(uint8_t*)p;
}
...
void WR_cpu_float_status(void* p, unsigned size, int v) {
    ...
    if ((void*)env < p && (p - (void*)env) < offsetof(CPUArchState, eip)) {
        __WR_env_raw(env, p - (void*)env, v, size);
    } else {
        assert(false && "unexpected calling context");
    }
}
...
target_ulong __RR_cpu_dyn(void* p, unsigned size) {
    if ((void*)env < p && (p - (void*)env) < offsetof(CPUArchState, eip)) {
        return __RR_env_raw(env, p - (void*)env, size);
    }
    assert(false && "unexpected calling context");
    return 0;
}
